...
def get_login_session_variabls(test_path_dict = None,valid_full_dep_path=None,test_result = None):
    ...
    for lgn in login_list:
        ...
        if lgn_depend_root:
            temp_login_list.append(lgn_depend_root)   #只添加正常登陆，否则test_path_dict[login]会增加None的key
...
def runTest():
    ...
    if not args.testset_paths:
        ...
        test_dep_dict, test_path_dict = dependency_parser.load_test_dependency_map_by_path(os.path.join(ROOT_DIR, Config.testcase_root_path,Config.testcase_files_path,Config.testcase_dep_parse_dir))
    ...
    for testcase,path in test_path_dict.items():
        assert testcase + '.yml' == os.path.basename(path), 'testcase_name and filename NOT consistent! testcase:{} != file_name:{},\n Check the file in abs_path:{}'.format(testcase, os.path.basename(path), path)
    ...
    for target_env in env_list:
        ...
        if args.testcases:
            logger.color_print("specified testcases is:{}".format(args.testcases))
            if isinstance(args.testcases, (list, set)):  # 若参数中指定用例名，则只跑指定的用例
                for testcase in args.testcases:
                    run_ydhrunner(testcase = testcase,valid_full_testcase_dependency= valid_full_testcase_dependency,
                                  test_path_dict= test_path_dict ,http_client_session_dict= http_client_session_dict,
                                  test_result= ydh_testresult,global_variables=global_variables)
        elif args.target_dirs:
            logger.color_print("specified target_dirs is:{}".format(args.target_dirs))
            for target_dir in args.target_dirs:
                target_dir_files = os.listdir(os.path.join(ROOT_DIR, target_dir))
                for test_file in target_dir_files:
                    for test, file_path in test_path_dict.items():
                        if os.path.basename(file_path) == test_file:
                            run_ydhrunner(testcase=test,valid_full_testcase_dependency=valid_full_testcase_dependency,
                                          test_path_dict=test_path_dict,http_client_session_dict=http_client_session_dict,
                                          test_result=ydh_testresult, global_variables=global_variables)
...
def run_ydhrunner(testcase,valid_full_testcase_dependency,test_path_dict,http_client_session_dict,test_result,global_variables):
    ...
    ydhrunner = YDHHttpRunner(test_path_dict, http_client_session_dict=http_client_session_dict,
                              test_result=test_result, dependent_root=login_dep_root,
