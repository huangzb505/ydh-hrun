...
def teardown_hook_resp(resp_obj,testcase_name):
    ...
    if Config.debug_output_enable:
        return_json_path =  os.path.join(ROOT_DIR, Config.debug_root_path, Config.return_json_path)
        if not os.path.exists(return_json_path):
            os.makedirs(return_json_path)

        test_path_dict_debug = utils.setup_debug_return_json_path ([Config.testcase_root_path,Config.testcase_files_path], [Config.debug_root_path, Config.return_json_path],
                                   src_dir = os.path.join(ROOT_DIR,Config.testcase_root_path,Config.testcase_files_path))
        testcase_name_json = testcase_name + '.json'
        print(test_path_dict_debug[testcase_name])
        with open(os.path.join(test_path_dict_debug[testcase_name], testcase_name_json), 'w') as f:
